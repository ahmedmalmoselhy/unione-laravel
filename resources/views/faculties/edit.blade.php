...
<div class="max-w-2xl">
    <div class="mb-6">
        ...
        <p class="mt-2 text-sm text-gray-600">Update faculty information</p>
    </div>

    <form action="{{ route('faculties.update', $faculty) }}" method="POST" class="bg-white shadow rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $faculty->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
